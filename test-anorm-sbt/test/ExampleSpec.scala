...
class ExampleSpec extends PlaySpec with OneAppPerSuite {
  ...
  object Util {
    ...
    def parser[T](
      ...
      value match {
        case json: com.aliyun.polardb2.util.PGobject => parseJson(f, columnName.qualified, json.getValue)
        case _=> {
          Left(
            TypeDoesNotMatch(
              s"Column[${columnName.qualified}] error converting $value to Json. Expected class to be[com.aliyun.polardb2.util.PGobject] and not[${value.asInstanceOf[AnyRef].getClass}"
            )
          )
        }
