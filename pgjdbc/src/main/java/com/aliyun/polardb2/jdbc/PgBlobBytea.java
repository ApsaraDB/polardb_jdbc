...
public class PgBlobBytea implements java.sql.Blob {
  ...
  public int setBytes(long pos, byte[] bytes, int offset, int len) throws SQLException {
    ...
    if (offset + len > bytes.length) {
      throw new PSQLException(GT.tr("Invalid offset/len parameters {0}/{1} for length {2}",
  ...
  public void truncate(long len) throws SQLException {
    ...
    if (len <= data.length) {
      ...
      throw new PSQLException(GT.tr("Called truncate with length too long for string of length "
          + "{0}", data.length),
          PSQLState.INVALID_PARAMETER_VALUE);
    }
  }
